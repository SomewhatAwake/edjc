...
fn default_cache_timeout() -> u64 { 300 } // 5 minutes
fn default_neutron_threshold() -> f64 { 500.0 }
fn default_white_dwarf_threshold() -> f64 { 150.0 }
fn default_result_format() -> String {
    "🚀 {jumps} jumps to {system} ({distance:.1}ly) via {route}".to_string()
}
fn default_show_fuel() -> bool { false }
fn default_show_time() -> bool { false }
...
pub fn create_sample_config() -> Result<()> {
    ...
    if config_path.exists() {
        return Err(anyhow!("Configuration file already exists at: {:?}", config_path));
    }
...
mod tests {
    ...
    fn test_config_serialization() {
        ...
        assert_eq!(config.cache_timeout_seconds, deserialized.cache_timeout_seconds);
